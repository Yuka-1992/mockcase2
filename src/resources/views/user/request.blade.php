  ...
  <div class="tab-content" id="pending-tab">
    <table class="list-table">
      ...
      <tbody>
        ...
        <tr>
          <td>承認待ち</td>
          <td>{{ $request->user_name ?? Auth::user()->name }}</td>
          <td>{{ $request->target_date }}</td>
          <td>{{ $request->reason }}</td>
          <td>{{ $request->created_at }}</td>
          <td>
            @if($request->attendance_id)
              <a href="{{ route('user.show', ['id' => $request->attendance_id]) }}" class="detail-link">詳細</a>
            @elseif($request->target_date)
              <a href="{{ route('user.show', ['id' => 0, 'date' => $request->target_date]) }}" class="detail-link">詳細</a>
            @else
              -
            @endif
          </td>
